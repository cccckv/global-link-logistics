...
export async function uploadRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/upload/receipt', {
    ...
    const uploadDir = path.join(process.cwd(), 'uploads', 'receipts', year, month);
    await fs.mkdir(uploadDir, { recursive: true });
    ...
    const fileUrl = `/api/uploads/receipts/${year}/${month}/${filename}`;
  ...
  fastify.get('/uploads/receipts/:year/:month/:filename', async (request, reply) => {
    try {
    ...
    const filePath = path.join(process.cwd(), 'uploads', 'receipts', year, month, safeFilename);
