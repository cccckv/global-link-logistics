...
| :--- | :--- | :--- | :--- |
| **1. 订舱委托** | `DRAFT` | 客户下整柜委托，确定箱型与整柜报价 | 客户唛头 (`userMark`)、**国内起运港口 (`originPort`)** ➔ 目的港口、箱型规格 (`20GP/40HQ`)、整柜包干报价 (`CLIENT_QUOTATION`，**选填/待定，支持后续详情页随时补录**) |
...
## 💰 6. 整柜协议报价待定与订舱海运费 (BOOKING FEE) 预警看板规范

### 6.1 整柜包干协议总报价待定规则
- **非必填放开**：在【单票下单工作台】与【Excel 批量导入】时，海运整柜协议总报价（`CLIENT_QUOTATION` / `fixedPriceAmount`）支持待定留空，严禁因报价未定而整单跳过阻断业务排柜装运；
- **财务零账与负毛利预警**：报价留空时，运单总应收金额暂计为 `¥0.00`，待发生订舱费等干线成本时毛利预警显示为负数；
- **详情页醒目预警与一键补录**：运单全生命周期详情页顶部高亮展示 Alert Banner，财务收支卡片总应收旁标注 `待补录报价`，支持操作员随时一键录入币种与金额，保存后系统自动触发财务重算（`calculateWaybillFinancials`）并锁定最终利润。

### 6.2 集装箱跟踪界面未录订舱海运费 (BOOKING FEE) 快捷查询规范
- **业务价值**：订舱海运费是整柜干线核心成本（几百至几千美金），防止开船后跟单员漏录导致财务利润失真；
- **判定准则**：集装箱在 `ContainerFee` 中没有 `feeSubject = 'BOOKING_FEE'` 且金额大于 0 的记录；
- **快捷药丸交互 (Quick Pill)**：
  - 工具栏提供 `[ ⚠️ 待录订舱海运费 (X) ]` 快捷切换药丸，角标动态呈现待录货柜总数，一键直达过滤；
  - 列表卡片上针对漏录货柜醒目标注 `待录订舱费` 按钮，点击一键直达录费弹窗并自动预选 `BOOKING_FEE` 科目与 `USD` 币种。
