...
### 6.3 整柜干线杂费录入当日实时汇率自动引入准则
- **基准汇率动态拉取**：打开“录入整柜干线全链路成本”弹窗时，系统自动拉取当日外汇牌价（`financeV2Api.getTodayExchangeRates()`），杜绝前端硬编码静态汇率；
- **币种折算基准匹配**：
  - **USD（美元）**：根据贵币计价法直接套用今日基准汇率 `todayRates.usdRate`（如 7.2345）；
  - **PHP（菲律宾比索）**：牌价基准为 1 CNY = X PHP，折算为人民币系数自动取倒数 `(1 / todayRates.phpRate).toFixed(4)`（如 1 / 8.0000 = 0.1250）；
  - **CNY（人民币）**：折算汇率固定为 1.0；
- **联动切换与重置保护**：
  - 切换费用科目（如 `BOOKING_FEE` ➔ `USD`，`THC_OVERSTAY_FEE` ➔ `PHP`，`PORT_SURCHARGE` ➔ `CNY`）或手动切换币种时，系统自动联动带出最新当日基准汇率；
  - 弹窗直观展示当日汇率数据来源与日期（如 `2026-09-10 实时接口/今日缓存`），并提供【重置当日汇率】按钮以应对人工微调后的复位；
  - 金额输入时实时计算并渲染 `折合人民币预估: ¥xxx.xx`，保障录入人员财务直观核算准确。

---

## 💎 7. 海运整柜 (SEA_FCL) 全链路成本多端互通与对账穿透规范 (FCL Cost Synchronization Standard)

在海运整柜（`SEA_FCL`）业务中，集装箱与运单是 1:1 独占强绑定的。集装箱干线履约（订舱、港杂、清关、拖车）以及还柜时产生的全部费用，**天然构成该整柜订单的履约应付成本**。坚决杜绝“模块各自为政、整柜零成本、虚假高毛利”现象。

### 7.1 运单详情页（WaybillDetailView）“单入口 + 智能归流”规范
- **杜绝多按钮混淆**：运单详情页右侧财务区**保持单一统一的【+ 录入费用】按钮**，严禁并列出现“添加杂费”与“录入整柜成本”两个按钮；
- **标准化科目分组选择**：
  - 当运单为 `SEA_FCL` 且已绑定货柜时，弹窗“费用科目”提供两组清晰的选项：
    1. **📦 整柜干线履约类（自动存入 `ContainerFee` 关联货柜）**：海运订舱费 (BOOKING_FEE)、出口港杂费 (PORT_SURCHARGE)、出口拖车费 (TRUCKING_FEE)、出口报关费 (CUSTOMS_FEE)、目的港清关费 (CLEARANCE_FEE)、送柜拖车费 (DEST_TRUCKING_FEE)、超期滞箱/修箱费 (THC_OVERSTAY_FEE)；
    2. **📝 单票自定附加收支（存入 `WaybillFee` 关联运单）**：代收国内车费、单票送货小费、单票理赔与客户垫付款等；
- **全量成本穿透与真实毛利聚合**：
  - 运单详情页的费用清单自动聚合展示该单名下全部 `WaybillFee` 与关联货柜的全部 `ContainerFee`；
  - 无论该笔费用是在【运单详情】、【集装箱整柜跟踪】还是【空箱还柜管理】录入，三端实时同步、数据完全一致；
  - 运单详情顶部应付与纯毛利实时联动计算真实净利润：`真实纯毛利 = 客户协议总应收 - (整柜干线全量成本 + 单票杂费)`。

### 7.2 财务核算与对账工作台（FinanceWorkbench）整柜穿透与结清规范
- **右侧应付明细自动穿透**：针对 `SEA_FCL` 订单，抽屉右侧【应付款项明细】自动穿透拉取关联集装箱在全链路发生的所有 `ContainerFee`（带上 `[整柜干线]`、`[港杂清关]`、`[还柜异常]` 等明确标识，含原币、单票汇率与折合人民币）；
- **逐笔确认付款与凭证归档**：财务可直接对穿透展示的整柜成本项点击【确认付款】，选择付款渠道（如银行对公、菲律宾银行电汇），归档出纳水单并锁定结清；
- **补录应付成本弹窗标准化**：
  - 规范输入：取消自由文本输入，下拉提供标准整柜成本科目；
  - 双向写回：财务在此补录的整柜成本，系统自动写回该集装箱的 `ContainerFee`，确保集装箱跟踪与财务对账保持同一本大账；
- **底栏与顶部 KPI 真实扣减**：财务对账工作台顶部看板（折合总应付、综合纯毛利）与抽屉底栏统计，必须全量计入整柜干线成本，彻底解决“整柜订单应付为 ¥0.00、毛利虚高 100%”的失真问题。
