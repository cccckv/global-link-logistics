...
### 3.4 当日实时汇率自动化与单票人工微调机制
1. **系统自动获取当日汇率**：
   - 系统内置汇率自动获取服务（`ExchangeRateService`），每日自动查询基准外汇行情；
   - 具备安全熔断兜底机制：若接口网络异常，自动降级为系统基准安全值（USD 7.20 / PHP 8.00），业务 100% 不中断；
   - 提供后端查询接口 `/api/v2/finance/exchange-rate/today`。
2. **单票人工微调（仅对当前单生效）**：
   - 手工录单或批量导入时，若业务员/财务未填写汇率，系统默认自动填充当日最新汇率；
   - 针对特批客户或特定单票，允许直接在订单表头微调 `usdRate` 或 `phpRate`；
   - **坚决不设月度默认汇率**：不同订单出运与结汇时间离散，统一月度默认极易与实际行情脱节引发亏损，因此坚持“当日参考基准 + 单票手工微调”模式。
