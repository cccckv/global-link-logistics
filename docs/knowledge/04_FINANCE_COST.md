...
- **整柜订单（`SEA_FCL`）标准化科目与双向写回**：针对整柜订单，弹窗取消自由文本输入，改为标准化整柜履约科目下拉（海运订舱费、报关费、清关税费、THC码头费、超期滞箱费等）。录入的成本自动双向同步写入关联集装箱的 `ContainerFee`。
...
#### ⑤ 海运整柜 (`SEA_FCL`) 干线成本自动穿透核销机制
- **彻底杜绝整柜“零成本、虚假高毛利”**：
  - 海运拼箱（`SEA_LCL`）：维持现状，大柜成本公摊，不直接穿透到散货单票中；
  - 海运整柜（`SEA_FCL`）：右侧应付区域**自动穿透拉取关联集装箱名下的全部全链路成本（`ContainerFee`）**，包含订舱费、报关清关、码头港杂以及在空箱还柜录入的滞箱费；
- **逐笔独立付款结清**：穿透的整柜成本项支持财务直接逐笔【确认付款】并归档出纳水单；
- **毛利与 KPI 真实计算**：工作台顶部 KPI 与该订单纯毛利，真实扣减全部整柜履约干线成本。
