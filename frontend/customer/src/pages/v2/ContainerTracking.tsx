...
export default function ContainerTracking() {
  ...
  const [filterMissingBookingFee, setFilterMissingBookingFee] = useState<boolean>(false);
  const [missingBookingFeeCount, setMissingBookingFeeCount] = useState<number>(0);
  ...
  const loadContainers = async (
    ...
    try {
      ...
      const isMissingFee = overrideMissingFee !== undefined ? overrideMissingFee : filterMissingBookingFee;
      ...
      const res = await containerV2Api.list({
        ...
        missingBookingFee: isMissingFee ? 'true' : undefined,
      });
      ...
      if (res.data.success) {
        ...
        if (res.data.counts?.missingBookingFee !== undefined) {
          setMissingBookingFeeCount(Number(res.data.counts.missingBookingFee) || 0);
        }
  ...
  const handleClearSearch = () => {
    ...
    setFilterMissingBookingFee(false);
    setSearchParams({});
    loadContainers('', 'ALL', 'ALL', 'ALL', false);
  };
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 space-y-4">
        <form onSubmit={handleSearchSubmit} className="flex flex-col md:flex-row gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* 方案 A: 待录订舱海运费 (BOOKING FEE) 快捷药丸 */}
            <button
              type="button"
              onClick={() => {
                const next = !filterMissingBookingFee;
                setFilterMissingBookingFee(next);
                loadContainers(searchQuery, selectedStatus, selectedOriginPort, selectedDestPort, next);
              }}
              className={`px-3 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 border shadow-xs ${
                filterMissingBookingFee
              ...
              title="一键筛选未录入订舱海运费 (BOOKING FEE) 的集装箱货柜"
            >
              <AlertCircle className={`w-3.5 h-3.5 ${filterMissingBookingFee ? 'text-white' : 'text-amber-500'}`} />
              <span>待录订舱海运费</span>
              <span
                className={`px-1.5 py-0.2 rounded-full text-[10px] font-mono font-bold ${
                  filterMissingBookingFee
                    ? 'bg-white/25 text-white'
                    : missingBookingFeeCount > 0
                    ? 'bg-amber-100 text-amber-800'
                    : 'bg-slate-100 text-slate-500'
                }`}
              >
                {missingBookingFeeCount}
              </span>
            </button>
            ...
            {(searchQuery || selectedStatus !== 'ALL' || selectedOriginPort !== 'ALL' || selectedDestPort !== 'ALL' || filterMissingBookingFee) && (
              <button
                type="button"
        ...
        {(searchQuery || selectedStatus !== 'ALL' || selectedOriginPort !== 'ALL' || selectedDestPort !== 'ALL' || filterMissingBookingFee) && (
          <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-slate-100 text-xs text-slate-600">
            ...
            {filterMissingBookingFee && (
              <span className="px-2.5 py-1 bg-amber-50 text-amber-800 rounded-lg font-semibold flex items-center gap-1.5 border border-amber-200">
                <AlertCircle className="w-3.5 h-3.5 text-amber-600" />
                仅看待录订舱海运费 (BOOKING FEE)
                <button
                  type="button"
                  onClick={() => {
                    setFilterMissingBookingFee(false);
                    loadContainers(searchQuery, selectedStatus, selectedOriginPort, selectedDestPort, false);
                  }}
                  className="text-amber-500 hover:text-red-500 font-normal"
                >
                  ×
                </button>
              </span>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          containers.map((container) => {
            ...
            const hasBookingFee = container.fees?.some(
              (f) => f.feeSubject === 'BOOKING_FEE' && Number(f.amount) > 0
            );
